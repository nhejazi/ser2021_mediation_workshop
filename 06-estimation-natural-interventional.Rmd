...
- For other parameters, we will teach you how to use our packages _medoutcon_ and _medshift_
...
1. Fit models for $g(a\mid w)$, $e(a\mid m, w)$, and $b(a, m, w)$
     - In this example we will use Generalized Additive Models [CITE] for tractability
     - In applied settings we recommend using an ensemble of data-adaptive regression algorithms, such as the Super Learner [CITE]
```{r, echo = TRUE, eval = TRUE, cache = TRUE}
library(mgcv)
...
2. Compute predictions  $g(1\mid w)$, $g(0\mid w)$, $e(1\mid m, w)$, $e(0\mid m, w)$,$b(1, m, w)$, $b(0, m, w)$, and  $b(a, m, w)$
```{r, echo = TRUE, eval = TRUE, cache = TRUE}
