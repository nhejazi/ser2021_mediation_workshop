...
- Let $Q(M, W)$ denote $\E(Y\mid A=1, M, W) - b(Y\mid A=0, M, W)$
...
- Estimating $\P(M\mid A, W)$ is a really hard problem when $M$ is high-dimensional. But,
...
\begin{equation*}
  \frac{\P(M\mid A=0,W)}{\P(M\mid A=1,W)} = \frac{\P(A = 0 \mid M, W) \P(A=1 \mid W)}{\P(A = 1 \mid M, W)\P(A=0 \mid W)}.
\end{equation*}
...
one_step <- function(y, m, a, w) {
  ...
  q_pred <- predict(q_fit, newdata = data.frame(a = 0, w = w))
  weights <- a / g0_pred * e0_pred / e1_pred - (1 - a) / g0_pred
  eif <- weights * (y - b_pred) + (1 - a) / g0_pred * (pseudo - q_pred) + q_pred
  return(mean(eif))
}
```
...
```{r, echo = FALSE, eval = FALSE, fig.height = 8, fig.width = 8}
cis <- cbind(
