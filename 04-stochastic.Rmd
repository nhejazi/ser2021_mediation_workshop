...
- In both cases $A\mid W$ is non-deterministic post-intervention, thus the name _stochastic intervention_
...
- Intervention: for each individual, increase the odds of participating in a sports by $\delta=2$
...
plot(pscore, pscore_delta, xlab = 'Observed prop. score', ylab = 'Prop. score under intervention')
abline(0, 1)
...
- Still require the abscence of intermediate confounders
  ...
  - There is a version of these effects that can accomodate intermediate confounders [@hejazi2020nonparametric]
  - R implementation to be released soon, stay tuned!

<!--
ID: Should I add: continuous interventions, and/or the definitions of effects with intermediate confounders?
KER: I don't think adding for intermediate confounders is necessary. Continuous interventions would be really useful *if* practical methods existed for that, but I think despite the paper existing, it hasn't really been done in practice, right? So probably wouldn't want to get into that.
-->
